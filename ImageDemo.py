...
def image_demo(filename, save_images=True):
    ...
    trash = trash_detector_gpt.perform_trash_detection(gpt_img_input, image.shape[0], image.shape[1])
    ...
    if save_images:
        ...
        if len(trash) > 0:
            for item in trash:
                ...
                y = round(item.location[0] / image.shape[0], 2)
    ...
    match_results = trash_match.match_results(trash, trash_result, image.shape[0], image.shape[1])
    
    if save_images:
        match_image = trash_detector_gpt.overlay_results(image, match_results)
        # save img
